...
def main():
    ...
    trips = read("trips.txt", dtype={"route_id": str, "service_id": str, "trip_id": str})
    cal = read("calendar_dates.txt", dtype={"service_id": str, "date": str})

    dates = target_dates()
    active_by_date = {}
    for d in dates:
        active_by_date[d] = set(cal[(cal["date"] == d) & (cal["exception_type"] == 1)]["service_id"])
